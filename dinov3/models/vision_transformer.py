...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(self, x: Tensor, masks=None) -> Tuple[Tensor, Tuple[int]]:
        ...
        if masks is not None:
            masks = F.interpolate(masks, (H, W), mode='nearest')[:, 0].flatten(1, 2)
            x = torch.where(masks.unsqueeze(-1).bool(), self.mask_token.to(x.dtype).unsqueeze(0), x)
    ...
    def _get_intermediate_layers_not_chunked(self, x: Tensor, mask = None, n: int = 1) -> List[Tensor]:
        x, (H, W) = self.prepare_tokens_with_masks(x, mask)
    ...
    def get_intermediate_layers(
        ...
        *,
        ...
        outputs = self._get_intermediate_layers_not_chunked(x, mask, n)
        if norm:
            outputs_normed = []
            for out in outputs:
