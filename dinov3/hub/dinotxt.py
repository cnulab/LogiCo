...
def dinov3_vitl16_dinotxt_tet1280d20h24l(
    *,
    ...
    bpe_path_or_url: str = "dinov3/bpe_simple_vocab_16e6.txt",
    check_hash: bool = False,
    vision_backbone_pretrained_path = None,
    txt_backbone_pretrained_path = None,
) -> Tuple[nn.Module, Any]:
    ...
    if vision_backbone_pretrained_path is not None:
        vision_backbone_state_dict = torch.load(vision_backbone_pretrained_path, map_location='cpu')
        model.visual_model.backbone.load_state_dict(vision_backbone_state_dict)

    if txt_backbone_pretrained_path is not None:
        txt_backbone_state_dict = torch.load(txt_backbone_pretrained_path, map_location='cpu')
        model.load_state_dict(txt_backbone_state_dict, strict=False)
