...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, recon_token):
        B, C, H, W = x.shape
        N = H * W
        x = x.reshape(B, C, H * W).permute(0, 2, 1)
        recon_token = recon_token.reshape(B, C, H * W).permute(0, 2, 1)
        q = self.q(x).reshape(B, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)[0]
        kv = self.kv(recon_token).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
